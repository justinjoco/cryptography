...
'''
Generates encryption key based on ascii key txt file
'''
...
'''
Generates round key list based on encryption key
'''
...
'''
Changes one random bit in the plaintext
'''
def changePlainText(message):

	message_bv = BitVector(textstring = message)
	ret_message_bv = message_bv.deep_copy()
	randIndex = random.randint(0, len(message_bv))

	if (ret_message_bv[randIndex]) == 0:
		ret_message_bv[randIndex] = 1
	else:
		ret_message_bv[randIndex] = 0
	


	return ret_message_bv.get_bitvector_in_ascii()

'''
Encrypts a message based on an encryption key
'''
...
def encrypt(message, encryptionKey):
	...
	return msg_encrypted_bv

'''
Deciphers encrypted text based on key
'''
...
'''
Randomizes and changes the global s_boxes
'''
...
def changeSboxes():
	...
	for i in range(8):
		...
		new_s_boxes[i] = [random.sample(range(16), 16) for j in range(4)]
...
'''
Inverts a random bit in the key bitvector
'''
def changeKey(key):

	randIndex = random.randint(0, len(key))
	newKey = key.deep_copy()

	if newKey[randIndex] == 0:
		newKey[randIndex] = 1
	else:
		newKey[randIndex] = 0
...
def main():
	...
	encryptedBv1 = encrypt(message, key)
	
	
	print("Question 1: Plaintext Diffusion")
	newMessageList = []
	total = 0
	
	#Iterates through 3 trials of plaintext changes
	for count in range(3):
		
		newMessage = changePlainText(message)
		newMessageList.append(newMessage)
		encryptedBv2 = encrypt(newMessage, key)
		xor_bv = encryptedBv1 ^ encryptedBv2
		
		total+=xor_bv.count_bits()
		
	print("Average number of ciphertext bits changed due to one plaintext bit change (3 trials) = {}".format(total/3))

	print("")

	#Saves original s_boxes
	global s_boxes
	temp = s_boxes
	
	print("Question 2: Plaintext + S_Box Diffusion")
	#Diffusion Experiment: S-Boxes Set 
	...
	for i in range(2):
		...
		#Uses same new messages created earlier
		for messageElem in newMessageList:
			encryptedBv3 = encrypt(messageElem, key)
			xor_bv2 = encryptedBv1 ^ encryptedBv3
			total += xor_bv2.count_bits()
		print("Trial {}".format(i+1))
		print("Average number of ciphertext bits changed with random Sboxes and one plaintext bit change = {}".format(total/len(newMessageList)))
		



	#Resets s_boxes to contain original values
	s_boxes = temp
	
	

	print("")
	print("Question 3: Key Confusion")
	total = 0
	
	#Calculates avg number of ciphertext bit changes over 4 keys
	for i in range(4):
		newKey = changeKey(key)
		encryptedBv4 = encrypt(message, newKey)
		xor_bv3 = encryptedBv1 ^ encryptedBv4
		total += xor_bv3.count_bits()

	print("Average number of ciphertext bits changed with one key bit change (4 keys): {}".format(total/4))
	

	
	

	#Saves ciphertext from original message encryption, S_boxes and key
	cipherText = encryptedBv1.get_bitvector_in_hex()	
